...
type Notify struct {
	Msg    Op
	Result Err // is leader changed when Get/PutAppend
}
...
// ========================================================================================
// ============ Notification mechanism for rpc waiting for raft completed =================

// wait raft applyMsg finish after Start()
...
// put Msg into ck.messageCh, notify rpc handler that raft applied msg, can return
...
// called by testers when a ShardKV instance won't be needed again
...
func (kv *ShardKV) transferShardsToGroup(shards []int, servers []string, configNum int) bool {
	...
	for { // todo when all server access fail
		for _, servername := range servers {
			DPrintf("[ShardKV-%d-%d] start move shard args=%v", kv.gid, kv.me, args)
			ok := kv.make_end(servername).Call("ShardKV.RequestMoveShard", &args, &reply)
			if ok && reply.Err == OK {
	...
	if len(leaveOp.LeaveShards) != 0 {
		go func(shardReqId int64, op ShardChangeOp, reqSeqId int) {
			for gid, shards := range leaveOp.LeaveShards {
				...
				// 比如初始 group0 存了 0~9 shard, 加入group1和2后
				// shards: 4,5,6 -> servers: Servers[1]
				// shards: 7,8,9 -> servers: Servers[2]
...
// called by another server, caller should also allocate ClerkId and SeqId
...
// =====================================================================================
// ======================= go coroutines run in the background =========================
